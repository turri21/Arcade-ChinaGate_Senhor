...
// pause_overlay.sv — overlay pausa per ChinaGate (256x240, RGB 4-bit).
//
// Coord raster: hdump (HJUMP=1) visibile 4..255 + 0x180..0x183 (256 px totali).
//               vdump visibile 8..246 (240 lines).
//
// Layout:
...
module pause_overlay (
	...
	input  wire [3:0]  rgb_r_in,
	input  wire [3:0]  rgb_g_in,
	input  wire [3:0]  rgb_b_in,

	output wire [3:0]  rgb_r_out,
	output wire [3:0]  rgb_g_out,
	output wire [3:0]  rgb_b_out
);

// Effective overlay: pause attiva ma clean disattivato.
wire overlay_on = pause & ~clean;

// VBlank pulse: vdump attraversa VB_START=0xF7=247.
reg [8:0] render_y_d;
always @(posedge clk) render_y_d <= render_y;
wire vblank_pulse = (render_y == 9'd247) && (render_y_d == 9'd246);

// =====================================================================
// Logo placement: 48x48 sorgente, SCALE 2x → 96x96.
// Schermo 256x240 (hdump 4..255, vdump 8..246).
// Centro X = (4+255)/2 = 129, top-left X = 129-48 = 81
// Centro Y = (8+246)/2 = 127, top-left Y = 127-48 = 79
// =====================================================================
localparam [8:0] LOGO_X    = 9'd82;   // 81+1 (compensa shift sinistra)
...
always @(*) case (logo_pix)
	2'd0: {lr, lg, lb} = 12'h000;
	2'd1: {lr, lg, lb} = 12'hF0F;
	2'd2: {lr, lg, lb} = 12'h0EE;
	2'd3: {lr, lg, lb} = 12'hFFF;
endcase

// Logo sempre opaque (incluso pal0=nero), come CityConnection.
...
pause_text #(
	...
	.H_CHARS      (1),
	.MSG_ROWS     (1),
	.ORIGIN_X     (10'd90),    // (4+255)/2 - 80/2 +1 = 90 (compensa tagli a sinistra)
	.ORIGIN_Y     (9'd16),     // top + 8 px margine
	.SCROLL_EN    (0),
...
pause_text #(
	...
	.H_CHARS       (24),
	.MSG_ROWS      (66),
	.ORIGIN_X      (10'd11),    // 10+1 (compensa tagli a sinistra)
	.ORIGIN_Y      (9'd32),
	.SCROLL_EN     (1),
	.SCROLL_PERIOD (3),
	.FONT_FILE     ("logo/font_darius.hex"),
...
//   tier 3 = oro (gold)
// =====================================================================
function [11:0] tier_color;
	input [1:0] tier;
	begin
		case (tier)
			2'd0: tier_color = 12'hFFF;
			2'd1: tier_color = 12'h0EE;
			2'd2: tier_color = 12'hF0F;
			2'd3: tier_color = 12'hFD0;
		endcase
	end
endfunction

wire [11:0] header_rgb = 12'hFD0;             // giallo/oro
wire [11:0] patron_rgb = tier_color(patron_tier);

// Priorità: header > patron > logo > dim > raw
wire        text_on  = header_on | patron_on;
wire [11:0] text_rgb = header_on ? header_rgb : patron_rgb;

// =====================================================================
// Output mux
// =====================================================================
wire [3:0] dim_r = {1'b0, rgb_r_in[3:1]};
wire [3:0] dim_g = {1'b0, rgb_g_in[3:1]};
wire [3:0] dim_b = {1'b0, rgb_b_in[3:1]};

assign rgb_r_out = !overlay_on              ? rgb_r_in :
                   text_on                  ? text_rgb[11:8] :
                   in_logo_now & logo_opaque ? lr        :
                                              dim_r;
assign rgb_g_out = !overlay_on              ? rgb_g_in :
                   text_on                  ? text_rgb[7:4]  :
                   in_logo_now & logo_opaque ? lg        :
                                              dim_g;
assign rgb_b_out = !overlay_on              ? rgb_b_in :
                   text_on                  ? text_rgb[3:0]  :
